...
def encrypt(text, passwd):
    """
        Генерирует 256-битный ключ, используя алгоритм генерации ключа PBKDF2 из пароля. 
        Используется случайная соль для генерации пароля (128-бит). 
        Эта соль должна храниться в выходных данных вместе с зашифрованным текстом,
        потому что без неё ключ расшифровки не может быть получен повторно, и расшифровка будет невозможна.
        Полученный ключ состоит из 64 шестнадцатеричных цифр (32 байта), представляющих 256-битное целое число.
    """
